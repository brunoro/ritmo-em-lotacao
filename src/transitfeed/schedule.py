...
class Schedule(object):
  ...
  def __init__(self, problem_reporter=None,
               db_filename=":memory:", check_duplicate_trips=False,
               gtfs_factory=None, migrate=True):
    ...
    self.ConnectDb(db_filename, migrate)
  ...
  def ConnectDb(self, filename, migrate):
    def connector(db_file):
      print 'Connecting to database "%s"...' % filename
      if native_sqlite:
    ...
    try:
      self.db_file = tempfile.NamedTemporaryFile() if filename == ':memory:' else open(filename, 'rw')
      self._connection = connector(self.db_file.name)
    except sqlite.OperationalError:
      # Windows won't let a file be opened twce. mkstemp does not remove the
